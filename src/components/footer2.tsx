...
function AICoachIcon({ isActive }: { isActive: boolean }): JSX.Element {
  return (
    <View style={{ width: 20, height: 20, alignItems: "center", justifyContent: "center" }}>
      <RNText style={{ fontSize: 16, color: isActive ? "#a855f7" : "#6b7280" }}>🤖</RNText>
    </View>
  );
}
      ...
      <View className={`flex-row w-full px-2 pt-3 ${Platform.OS !== "web" ? "" : "pb-3"}`}>
        ...
        <View className="flex-row justify-around flex-1">
          ...
          <FooterButton
            name="ai"
            currentTab={currentTab}
            icon={(isActive) => <AICoachIcon isActive={isActive} />}
            text="AI Coach"
            onClick={() => props.dispatch(Thunk_pushScreen("aiCoach", undefined, { tab: "ai" }))}
          />
          <FooterButton
            name="me"
            currentTab={currentTab}
